...
for i in range(episodes):
    ...
    while not game.is_episode_finished():
        ...
        depth_buf = state[0].depth_buffer
        ldepth = depth_buf[int(resolution[1] / 2)][0]
        cdepth = depth_buf[int(resolution[1] / 2)][int(resolution[0] / 2)]
        rdepth = depth_buf[int(resolution[1] / 2)][resolution[0] - 1]
        """
        notyet = True
        leftbrick = 0
        for i in range(0, resolution[0] - 1):
                if notyet:
                     leftbrick = i   
                     if depth_buf[int(resolution[1] / 2)][i] > 5:
                             notyet = False

        print("wall ends at: ", leftbrick)
        """
        #print(ldepth, cdepth, rdepth)
        #print("right depth: ", total_depth / resolution[1])
        ...
        #print("r = ", reward, "health = ", state[0].game_variables[0])
